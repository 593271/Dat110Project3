...
public class Util {
	 ...
	 */
	public static boolean computeLogic(BigInteger id, BigInteger lower, BigInteger upper) throws NoSuchAlgorithmException {
		...
		if(lower.compareTo(id) <= 0 && id.compareTo(upper) <= 0 || id.compareTo(upper) == 0 || id.compareTo(lower) ==0) {
			cond = true;
			
		}else if(lower.compareTo(upper) ==1) {
			if(id.compareTo(lower) >= 0 && id.compareTo(mod) <= 0 || id.compareTo(upper) <= 0 ) {
				cond = true;
			}
