...
public class FileManager {
	...
	public void createReplicaFiles() throws NoSuchAlgorithmException {
		...
		for(int i = 0; i < Util.numReplicas; i++) {
			String navn = getFilename()+i;
			BigInteger hash =Hash.hashOf(navn);
			replicafiles[i] = hash;
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException, NoSuchAlgorithmException {
		...
		for(int i = 0; i < replicafiles.length; i++) {
    		NodeInterface node = chordnode.findSuccessor(replicafiles[i]);    
    		Message mess = node.getFilesMetadata(replicafiles[i]);
    		succinfo.add(mess);
	...
	public NodeInterface findPrimaryOfItem() throws RemoteException, NoSuchAlgorithmException {
		...
		Set<Message> peers = requestActiveNodesForFile(filename);
        // iterate over the activeNodesforFile
        for (Message message : peers) {
            if (message.isPrimaryServer()) {
            	BigInteger primary = message.getNodeID();
            	NodeInterface succ = chordnode.findSuccessor(primary);
            	NodeInterface pred = succ.getPredecessor();
                return pred; 
            }
