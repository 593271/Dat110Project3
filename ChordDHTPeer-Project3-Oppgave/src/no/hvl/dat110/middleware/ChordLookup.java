...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException, NoSuchAlgorithmException {
		...
		succNode = Util.getProcessStub(succNode.getNodeName(), succNode.getPort());
		// check that key is a member of the set {nodeid+1,...,succID} i.e. (nodeid+1 <= key <= succID) using the ComputeLogic
		

		if (Util.computeLogic(key, node.getNodeID().add(BigInteger.ONE), succNode.getNodeID())) {
			...
			succNode = findHighestPredecessor(key);
			return succNode.findSuccessor(key);
		}
